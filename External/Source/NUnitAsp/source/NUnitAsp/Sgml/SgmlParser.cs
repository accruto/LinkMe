    ...
    public class Entity {
        ...
        public string ScanToken(StringBuilder sb, string term, bool nmtoken) {
            ...
            while (ch != Entity.EOF && term.IndexOf(ch)<0) {
                if (!nmtoken || ch == '_' || ch == '.' || ch == '-' || ch == ':' || IsLatin1LetterOrDigit(ch))
                {
                    sb.Append(ch);
                } 
                else {
        ...
        private static bool IsLatin1LetterOrDigit(char ch)
        {
            // This works like char.IsLetterOrDigit() IF we only care about Latin1 characters and it's faster.
        ...
        {
            Debug.Assert(terminators == terminators.ToLower() && terminators == terminators.ToUpper(),
                "The terminator characters are expected to be all symbols, but some seem to be letters: "
                + terminators);
