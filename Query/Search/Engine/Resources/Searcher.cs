...
namespace LinkMe.Query.Search.Engine.Resources
{
    public class Searcher
    {
        ...
        static Searcher()
        {
            ...
            FacetHandlers.add(new SimpleFacetHandler(FieldName.ItemType));
        }
        ...
        public ResourceSearchResults Search(LuceneQuery query, LuceneFilter filter, IEnumerable<BrowseSelection> selections, SortField[] sorts, int skip, int take, bool includeFacets)
        {
        ...
        private static BrowseRequest CreateRequest(LuceneQuery query, LuceneFilter filter, IEnumerable<BrowseSelection> selections, SortField[] sorts, int skip, int take, bool includeFacets)
        {
            ...
            if (includeFacets)
            {
                request.setFacetSpec(FieldName.ItemType, CreateFacetSpec());
            }
        ...
        private ResourceSearchResults GetSearchResults(BrowseResult result, int skip, int take)
        {
            ...
            var searchResults = new ResourceSearchResults {TotalMatches = result.getNumHits()};
        ...
        private static IDictionary<T, int> GetFieldHits<T>(java.util.Map facetMap, string fieldName, Func<string, T> decode)
        {
