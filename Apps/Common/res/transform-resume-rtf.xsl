  ...
  <xsl:template match="education">
	...
	<xsl:template match="job">
		<xsl:choose>
          ...
          <!-- Dates -->
          <xsl:if test="(daterange/start and daterange/start != '')">
            <xsl:text>{\f1\fs16\cf1\insrsid3700768 \par Start Date \tab \tab }</xsl:text>
					</xsl:if>
          <xsl:if test="(daterange/end and daterange/end != '')">
          ...
          <xsl:text>}{\b\f36\fs20\cf1\insrsid6488721 \tab \tab </xsl:text>
