...
{
    public abstract class CreditTests
        : UnlockTests
    {
        ...
        public void TestVisible()
        {
            ...
            TestUnlock(isResumeVisible, isPhoneNumberVisible, hasPhoneNumbers);
        }
        ...
        private void TestUnlock(bool isResumeVisible, bool isPhoneNumberVisible, bool hasPhoneNumbers)
        {
            ...
            var model = Unlock(member.Id);
            if (CanContact != CanContactStatus.No && CanContact != CanContactStatus.YesIfHadCredit && isResumeVisible)
            {
                // Unlocking will change status.

                canContact = CanContactStatus.YesWithoutCredit;
                hasBeenAccessed = true;
                AssertCandidate(model, canContact, true, true, isPhoneNumberVisible, hasPhoneNumbers, member.PhoneNumbers);
            }
            else
            {
                if (!isResumeVisible)
