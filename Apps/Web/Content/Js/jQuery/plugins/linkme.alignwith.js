 ...
 * (LinkMe fork: Martin Funcich. No copyright.)
 *
 ...
 * Notes:
 *
 *   alignWidth includes element borders and padding in the width and height calculations, but excludes margins.
	...
	$.fn.extend({
		alignWith: function(elem, pos, options) {
			...
			return this.each(function(){
				...
				var marginX = parseInt(t.css('margin-left'), 10);
				var marginY = parseInt(t.css('margin-top'), 10);
				tX -= isNaN(marginX) ? 0 : marginX;
				tY -= isNaN(marginY) ? 0 : marginY;
				...
				// Check for attachment options
				if (op.appendToBody) {
					t.appendTo('body');
				}
				if (op.appendTo) {
					op.appendTo = $(op.appendTo);
					if (op.appendTo.css("position") != "static") {
						var offset = op.appendTo.offset();
						tCss.left -= offset.left;
						tCss.top -= offset.top;
					}
					op.appendTo.append(t);
				}
