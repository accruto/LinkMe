<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/MasterPages/Block.Page.master" Inherits="LinkMe.Apps.Asp.Mvc.Views.ViewPage<LinkMe.Web.Areas.Administrators.Models.Organisations.OrganisationCreditsModel>" %>
    ...
    <div class="section">
	    <div class="section-body">
		    <ul class="plain-actions actions">
                ...
                <li><%= Html.RouteRefLink("View all credit usage", OrganisationsRoutes.Usage, new { id = Model.Organisation.Id })%></li>
		    </ul>
	    </div>
    </div>
</asp:Content>
...
<%  using (Html.RenderForm())
    {
        ...
        <div class="shadowed-section section">
            ...
            <div class="section-body">
                ...
                <%= Html.ButtonsField().Add(new AddButton()) %>
...
<%  if (Model.Allocations[Model.Organisation.Id].Count == 0)
    { %>
                <div id="credits-header">
                    There are no credits allocated to this organisation.
                </div>
<%  }
    else
    { %>
...
<%      var allocations = Model.Allocations[Model.Organisation.Id];
        for (var index = 0; index < allocations.Count; ++index)
        {
            var allocation = allocations[index]; %>
                        ...
                        <tr id="<%= allocation.Id %>" class="item item_<%= (index % 2) == 0 ? "odd" : "even" %>">
                            ...
                            <td class="usage-column"><%= Html.RouteRefLink("Usage", OrganisationsRoutes.AllocationUsage, new { id = Model.Organisation.Id, allocationId = allocation.Id })%></td>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            linkme.administrators.credits.ready({
                urls: {
                    apiDeallocateUrl: '<%= OrganisationsRoutes.ApiDeallocate.GenerateUrl(new { id = Model.Organisation.Id }) %>'
                }
            });
        });
    </script>
