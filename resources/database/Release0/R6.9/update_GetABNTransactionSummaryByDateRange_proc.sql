...
BEGIN
	select  SUM(CASE
			WHEN transactions.TransactionType = 0 THEN  transactions.gstComponent
			...
			WHEN transactions.TransactionType = 4 THEN transactions.gstComponent
			ELSE 0 
			END) AS GST,
		sum(CASE
			WHEN transactions.TransactionType = 0 THEN  transactions.MoneyComponent
			...
			WHEN transactions.TransactionType = 4 THEN transactions.MoneyComponent
			ELSE 0 
			END) as Dollars,
		sum(CASE
